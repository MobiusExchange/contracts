...
import "../interfaces/IMobiusRouter.sol";

/**
 * @title MobiusRouter02
 * @notice Allows routing on different mobius pools
 ...
 */
contract MobiusRouter is Ownable, ReentrancyGuard, IMobiusRouter {
    ...
    function swapTokensForTokens(
        ...
        address to,
        uint256 deadline
    ) external override before(deadline) nonReentrant returns (uint256 amountOut, uint256 haircut) {
     ...
     * @return toAmount The potential final amount user would receive
     * @return haircut The total haircut that would be applied
     */
    function quotePotentialSwaps(address[] calldata tokenPath, address[] calldata poolPath, uint256 fromAmount)
        external
        view
        returns (uint256 toAmount, uint256 haircut)
    {
        ...
        // Check for pool duplicates
        for (uint256 i; i < poolPath.length; ++i) {
            for (uint256 j = i + 1; j < poolPath.length; ++j) {
                require(poolPath[i] != poolPath[j], "duplicate pool in path");
            }
        }
        ...
        for (uint256 i; i < poolPath.length; ++i) {
            // check if we're reaching the beginning or end of the poolPath array
            if (i != 0) {
                nextFromAmount = toAmount;
            }

            // make the swap with the correct arguments
            (toAmount, localHaircut) =
                IPool(poolPath[i]).quotePotentialSwap(tokenPath[i], tokenPath[i + 1], nextFromAmount);
